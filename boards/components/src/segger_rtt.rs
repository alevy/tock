...
macro_rules! segger_rtt_memory_component_static {
    () => {{
        ...
        let up_buffer = kernel::static_buf!(
            [u8; segger::DEFAULT_UP_BUFFER_LENGTH]
        );
        let down_buffer = kernel::static_buf!(
            [u8; segger::DEFAULT_DOWN_BUFFER_LENGTH]
        );
...
impl Component for SeggerRttMemoryComponent {
    type StaticInput = (
        ...
        &'static mut MaybeUninit<[u8; segger::DEFAULT_UP_BUFFER_LENGTH]>,
        &'static mut MaybeUninit<[u8; segger::DEFAULT_DOWN_BUFFER_LENGTH]>,
    );
    ...
    fn finalize(self, s: Self::StaticInput) -> Self::Output {
        ...
        let up_buffer =
            s.1.write([0; segger::DEFAULT_UP_BUFFER_LENGTH]);
        let down_buffer =
            s.2.write([0; segger::DEFAULT_DOWN_BUFFER_LENGTH]);
